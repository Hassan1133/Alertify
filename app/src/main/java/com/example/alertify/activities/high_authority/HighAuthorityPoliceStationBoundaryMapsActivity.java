...
import com.example.alertify.databinding.ActivityHighAuthorityPoliceStationBoundaryMapsBinding;
...
public class HighAuthorityPoliceStationBoundaryMapsActivity extends FragmentActivity implements OnMapReadyCallback, View.OnClickListener {

    private GoogleMap googleMap;
    private ActivityHighAuthorityPoliceStationBoundaryMapsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityHighAuthorityPoliceStationBoundaryMapsBinding.inflate(getLayoutInflater());
    ...
    private void searchLocation() {
        binding.boundaryMapSearchView.setOnQueryTextListener(new androidx.appcompat.widget.SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                if (NetworkUtils.isInternetAvailable(HighAuthorityPoliceStationBoundaryMapsActivity.this)) {
                    String location = binding.boundaryMapSearchView.getQuery().toString();
                    List<Address> addressList = null;

                    if (!location.isEmpty()) {
                        geocoder = new Geocoder(HighAuthorityPoliceStationBoundaryMapsActivity.this, Locale.getDefault());

                        try {
                            addressList = geocoder.getFromLocationName(location, 1);
                        } catch (IOException e) {
                            Toast.makeText(HighAuthorityPoliceStationBoundaryMapsActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                        ...
                        if (!addressList.isEmpty()) {
                            ...
                            Toast.makeText(HighAuthorityPoliceStationBoundaryMapsActivity.this, "Please select a valid location", Toast.LENGTH_SHORT).show();
                        }
                    }
                } else {
                    Toast.makeText(HighAuthorityPoliceStationBoundaryMapsActivity.this, "Check your internet connection", Toast.LENGTH_SHORT).show();
